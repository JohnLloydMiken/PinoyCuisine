export const recipes = 
[
{name: "Adobo",
  image: "/assets/images/adobo.jpg",
 instructions: 
 [
  "In a bowl, marinate the meat with soy sauce, garlic, and peppercorns for 30 minutes.",
  "Heat oil in a pan, then fry the meat until lightly browned.",
  "Add the marinade, vinegar, bay leaves, and water to the pan.",
  "Simmer for 30-40 minutes until the meat is tender.",
  "Adjust the flavor with sugar, if desired. Serve with steamed rice."
 ] ,
 ingredients: 
 [
  "1 kg chicken",
  "1/2 cup soy sauce",
  "1/2 cup vinegar",
  "6 garlic cloves, minced",
  "1 tsp black peppercorns",
  "3 bay leaves",
  "1 tbsp cooking oil",
  "1 cup water",
  "1 tsp sugar (optional)"
 ] 
},

{name: "Sinigang Na Baboy",
  image: "/assets/images/Sinigang na Baboy.jpg",
  instructions: 
  [
   "Boil pork in water with onion and tomatoes until tender.",
   "Add tamarind mix or paste.",
   "Add radish, string beans, and simmer for 5 minutes.",
   "Stir in kangkong and season with salt or fish sauce. Serve hot.",
  
  ] ,
  ingredients: 
  [
   "1 kg pork belly or ribs",
   "8 cups water",
   "1 packet tamarind soup mix (or fresh tamarind paste)",
   "1 onion, quartered",
   "2 tomatoes, quartered",
   "1 radish, sliced",
   "1 cup string beans, cut into 2-inch pieces",
   "1 cup kangkong (water spinach) or bok choy",
   "Salt or fish sauce to taste"
  ] 
 },
{name: "Kare-Kare",
  image: "/assets/images/Kare-Kare.jpg",
  instructions: 
  [
   "Boil the meat until tender, set aside.",
   "In a pot, sauté garlic and onion, then add annatto powder.",
   "Pour in water and peanut butter, stirring to combine.",
   "Add the boiled meat and vegetables, cooking until vegetables are tender.",
   "Thicken with cornstarch mixture. Serve with shrimp paste."
  
  ] ,
  ingredients: 
  [
   "1 kg oxtail or beef (can include tripe)",
   "1/2 cup peanut butter",
   "1 tbsp annatto powder",
   "2 cups water",
   "1 eggplant, sliced",
   "1 banana blossom, sliced",
   "1 cup string beans, cut into 2-inch pieces",
   "2 tbsp cornstarch, dissolved in water",
   "Shrimp paste (bagoong), for serving"
  ] 
 },
 {name: "Lechon Kawali",
  image: "/assets/images/Lechon Kawali.jpg",
  instructions: 
  [
   "Boil pork belly in water with salt, peppercorns, and bay leaves until tender.",
   "Let it cool, then air-dry or refrigerate overnight.",
   "Deep fry until golden and crispy. Chop into bite-sized pieces.",
   "Serve with lechon sauce or vinegar with garlic.",
   
  
  ] ,
  ingredients: 
  [
   "1 kg pork belly",
   "6 cups water",
   "2 tbsp salt",
   "1 tsp peppercorns",
   "3 bay leaves",
   "Oil for deep frying",
  ] 
 },
 {
  name: "Pancit Canton",
  image: "/assets/images/Pancit Canton (Stir-Fried Noodles).jpg",
  instructions: [
    "Sauté garlic and onion, then cook the chicken or pork.",
    "Add chicken stock, soy sauce, and oyster sauce. Bring to a boil.",
    "Add vegetables and noodles. Stir until noodles are tender.",
    "Serve with calamansi or lemon on the side.",
  ],
  ingredients: [
    "250g pancit canton noodles",
    "1/2 cup chicken or pork, sliced",
    "1 cup mixed vegetables (carrots, cabbage, snow peas)",
    "3 tbsp soy sauce",
    "2 tbsp oyster sauce",
    "1 onion, sliced",
    "4 garlic cloves, minced",
    "1 1/2 cups chicken stock",
  ],
},
{
  name: "Bulalo",
  image: "/assets/images/Bulalo.jpg",
  instructions: [
    "Boil beef shank in water with onion until tender (around 1.5–2 hours).",
    "Add corn and simmer for 10 minutes.",
    "Add green beans and cabbage, then season with salt and pepper.",
    "Serve hot with rice.",
  ],
  ingredients: [
    "1 kg beef shank with bone marrow",
    "10 cups water",
    "1 onion, quartered",
    "1 corn on the cob, sliced",
    "1 cup cabbage or pechay (bok choy)",
    "1/2 cup green beans",
    "Salt and pepper to taste",
  ],
},
{
  name: "Tinola",
  image: "/assets/images/Tinola.jpg",
  instructions: [
    "Sauté ginger and onion, then add chicken.",
    "Pour water and simmer until chicken is tender.",
    "Add papaya or sayote and cook until soft.",
    "Stir in malunggay leaves and season with fish sauce. Serve warm.",
  ],
  ingredients: [
    "1 kg chicken, cut into pieces",
    "6 cups water",
    "2-inch ginger, sliced",
    "1 onion, quartered",
    "1 green papaya or sayote (chayote), sliced",
    "1 cup malunggay (moringa leaves) or spinach",
    "Fish sauce to taste",
  ],
},
{
  name: "Bicol Express",
  image: "/assets/images/Bicol Express.jpg",
  instructions: [
    "Sauté garlic, then add pork and cook until lightly browned.",
    "Stir in shrimp paste and coconut milk, simmering for 15 minutes.",
    "Add chilies and cook until pork is tender and sauce thickens.",
    "Serve with steamed rice.",
  ],
  ingredients: [
    "500g pork belly, sliced",
    "1 can coconut milk",
    "2 tbsp shrimp paste (bagoong)",
    "4 long green chilies, sliced",
    "1 red chili (optional, for spice)",
    "3 garlic cloves, minced",
  ],
},
{
  name: "Lumpiang Shanghai",
  image: "/assets/images/Lumpiang Shanghai.jpg",
  instructions: [
    "Mix pork, carrot, onion, garlic, egg, and breadcrumbs.",
    "Wrap 1 tbsp of mixture in a lumpia wrapper, sealing the edges.",
    "Deep fry until golden brown.",
    "Serve with sweet chili sauce.",
  ],
  ingredients: [
    "500g ground pork",
    "1 carrot, grated",
    "1 onion, minced",
    "2 garlic cloves, minced",
    "1 egg",
    "1/2 cup breadcrumbs",
    "Lumpia wrappers",
    "Oil for frying",
  ],
},
{
  name: "Halo-Halo",
  image: "/assets/images/Halo-Halo.jpg",
  instructions: [
    "Layer sweetened banana, beans, nata de coco, and other ingredients in a tall glass.",
    "Add crushed ice on top.",
    "Drizzle with evaporated milk and top with ube ice cream.",
  ],
  ingredients: [
    "Crushed ice",
    "1/2 cup sweetened banana (saba)",
    "1/2 cup sweetened beans (red mung or white kidney)",
    "1/2 cup nata de coco (coconut gel)",
    "1/2 cup leche flan (optional)",
    "1 scoop ube ice cream",
    "1/4 cup evaporated milk",
  ],
},

];



